...
| `VITE_ALLOWED_EXTERNAL_ORIGINS` | _(unset)_ | Comma-separated origins the app may link or redirect to beyond its own (suite-sibling switcher, Consumed-by link, whitelabel logo/hero/favicon, SCM OAuth redirect). Matched on full origin — scheme, host **and** port. Build-time on purpose: it guards against a compromised or misconfigured backend, so it must not be backend-supplied. When unset, any `http(s)` origin is accepted (dangerous URI schemes are still rejected) — see SECURITY.md. |
