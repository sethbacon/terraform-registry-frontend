 ...
 *  - Webhook events panel is present on modules with an SCM link
 */
...
test.describe('Module detail page', () => {
  ...
  test('webhook events panel is visible on modules with an SCM link', async ({ loggedInPage: page }) => {
    ...
    // The webhook events panel is only shown when the module has an SCM link.
    // Check if the panel exists; skip if this module has no SCM link.
    const webhookPanel = page.getByRole('heading', { name: /Webhook Events/i });
    const hasWebhookPanel = await webhookPanel.isVisible({ timeout: 5_000 }).catch(() => false);
    test.skip(!hasWebhookPanel, 'This module has no SCM link — skipping webhook events test');
  ...
  test('webhook events panel expands and shows events or empty state', async ({ loggedInPage: page }) => {
    ...
    await webhookPanel.click();

    // After expanding, wait for the loading spinner inside the panel (if any) to clear
    await page.waitForTimeout(500); // brief pause for Collapse animation
    const innerSpinner = page.locator('[class*="MuiCircularProgress"]').first();
    const innerSpinnerVisible = await innerSpinner.isVisible({ timeout: 3_000 }).catch(() => false);
    if (innerSpinnerVisible) {
      await expect(innerSpinner).toBeHidden({ timeout: 15_000 });
    }

    // Either event list items or the "No webhook events" empty state should appear
    const hasEvents = (await page.locator('[class*="MuiListItem"]').count()) > 0;
    const hasEmptyState = await page
      .getByText(/no webhook events/i)
      .isVisible()
      .catch(() => false);
