...
import {
  Box, Typography, Stack, Chip, TextField, FormControlLabel, Switch, FormControl,
  InputLabel, Select, MenuItem, Alert, Button, CircularProgress,
} from '@mui/material';
...
const StorageStep: React.FC = () => {
  const {
    ...
    storageTesting, storageTestResult, storageSaving, storageSaved,
    testStorage, saveStorage, goToStep,
  } = useSetupWizard();
    ...
    <Box>
      ...
      <Stack spacing={2}>
        ...
        {storageForm.backend_type === 'azure' && (
          <>
            <TextField fullWidth label="Account Name" value={storageForm.azure_account_name || ''}
              onChange={(e) => setStorageForm({ ...storageForm, azure_account_name: e.target.value })} required />
            <TextField fullWidth label="Account Key" type="password" value={storageForm.azure_account_key || ''}
              onChange={(e) => setStorageForm({ ...storageForm, azure_account_key: e.target.value })} required />
            <TextField fullWidth label="Container Name" value={storageForm.azure_container_name || ''}
              onChange={(e) => setStorageForm({ ...storageForm, azure_container_name: e.target.value })} required />
            <TextField fullWidth label="CDN URL (optional)" value={storageForm.azure_cdn_url || ''}
        ...
        {storageForm.backend_type === 's3' && (
          <>
            <TextField fullWidth label="Region" value={storageForm.s3_region || ''}
              onChange={(e) => setStorageForm({ ...storageForm, s3_region: e.target.value })} placeholder="us-east-1" required />
            <TextField fullWidth label="Bucket" value={storageForm.s3_bucket || ''}
              onChange={(e) => setStorageForm({ ...storageForm, s3_bucket: e.target.value })} required />
            <TextField fullWidth label="Endpoint (optional, for S3-compatible services)" value={storageForm.s3_endpoint || ''}
            ...
            {(storageForm.s3_auth_method === 'access_key' || !storageForm.s3_auth_method) && (
              <>
                <TextField fullWidth label="Access Key ID" value={storageForm.s3_access_key_id || ''}
                  onChange={(e) => setStorageForm({ ...storageForm, s3_access_key_id: e.target.value })} />
                <TextField fullWidth label="Secret Access Key" type="password" value={storageForm.s3_secret_access_key || ''}
                  onChange={(e) => setStorageForm({ ...storageForm, s3_secret_access_key: e.target.value })} />
              </>
            )}
            {storageForm.s3_auth_method === 'assume_role' && (
              <>
                <TextField fullWidth label="Role ARN" value={storageForm.s3_role_arn || ''}
                  onChange={(e) => setStorageForm({ ...storageForm, s3_role_arn: e.target.value })} />
                <TextField fullWidth label="Role Session Name" value={storageForm.s3_role_session_name || ''}
        ...
        {storageForm.backend_type === 'gcs' && (
          <>
            <TextField fullWidth label="Bucket" value={storageForm.gcs_bucket || ''}
              onChange={(e) => setStorageForm({ ...storageForm, gcs_bucket: e.target.value })} required />
            <TextField fullWidth label="Project ID" value={storageForm.gcs_project_id || ''}
              onChange={(e) => setStorageForm({ ...storageForm, gcs_project_id: e.target.value })} required />
            <FormControl fullWidth>
              <InputLabel>Auth Method</InputLabel>
              <Select
            ...
            {storageForm.gcs_auth_method === 'credentials_file' && (
              <TextField fullWidth label="Credentials File Path" value={storageForm.gcs_credentials_file || ''}
                onChange={(e) => setStorageForm({ ...storageForm, gcs_credentials_file: e.target.value })} />
            )}
            {storageForm.gcs_auth_method === 'credentials_json' && (
              <TextField fullWidth label="Credentials JSON" value={storageForm.gcs_credentials_json || ''}
                onChange={(e) => setStorageForm({ ...storageForm, gcs_credentials_json: e.target.value })} multiline rows={4} />
            )}
          </>
        )}
