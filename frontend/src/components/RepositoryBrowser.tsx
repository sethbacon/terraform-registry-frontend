...
const RepositoryBrowser: React.FC<RepositoryBrowserProps> = ({
  ...
  const loadRepositories = useCallback(async () => {
    try {
      ...
      if (status === 401 || status === 403) {
        setError(
          serverMessage ||
            'OAuth token is invalid or has been revoked; please reconnect to this SCM provider in Admin → SCM Providers.',
        )
      } else {
        setError(
          serverMessage
  ...
  const loadTagsAndBranches = useCallback(
    async (_repository: SCMRepository) => {
      try {
        setLoadingTags(true)
        setError(null)
        ...
        <Box>
          ...
          {nameFilter && hiddenCount > 0 && (
            <Alert severity="info" sx={{ mb: 2 }}>
              {hiddenCount} {hiddenCount === 1 ? 'repository' : 'repositories'} hidden — must match
              Terraform naming convention: <strong>terraform-&lt;provider&gt;-&lt;name&gt;</strong>{' '}
