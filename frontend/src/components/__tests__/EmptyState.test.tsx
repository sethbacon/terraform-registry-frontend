...
describe('EmptyState', () => {
  ...
  it('fires primaryAction on click', () => {
    const onClick = vi.fn()
    render(<EmptyState title="t" primaryAction={{ label: 'Do it', onClick }} />)
  ...
  it('fires secondaryAction on click', () => {
    const onClick = vi.fn()
    render(<EmptyState title="t" secondaryAction={{ label: 'Learn more', onClick }} />)
    fireEvent.click(screen.getByTestId('empty-state-secondary'))
    expect(onClick).toHaveBeenCalled()
  })

  it('accepts a custom icon', () => {
    render(<EmptyState title="t" icon={<svg data-testid="custom-icon" />} />)
