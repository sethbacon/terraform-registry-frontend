...
describe('ConfirmDialog', () => {
  ...
  it('renders severity icons for each level', () => {
    const { rerender } = render(<ConfirmDialog open onClose={vi.fn()} title="T" severity="info" />)
  ...
  it('renders fields and passes collected values to onSubmit', async () => {
    ...
    render(
      <ConfirmDialog
        ...
        onSubmit={onSubmit}
        title="Deprecate"
        fields={[{ id: 'message', label: 'Deprecation message', multiline: true, required: true }]}
  ...
  it('keeps loading state while onConfirm promise is pending', async () => {
    ...
    render(<ConfirmDialog open onClose={vi.fn()} onConfirm={onConfirm} title="T" />)
    const confirm = screen.getByRole('button', { name: 'Confirm' })
    await user.click(confirm)
    expect(confirm).toBeDisabled()
  ...
  it('onClose is called when cancel is clicked', async () => {
    const user = userEvent.setup()
    const onClose = vi.fn()
    render(<ConfirmDialog open onClose={onClose} onConfirm={vi.fn()} title="T" />)
    await user.click(screen.getByRole('button', { name: 'Cancel' }))
    expect(onClose).toHaveBeenCalledTimes(1)
  })

  it('respects external loading prop', () => {
    render(<ConfirmDialog open onClose={vi.fn()} onConfirm={vi.fn()} title="T" loading />)
    expect(screen.getByRole('button', { name: 'Confirm' })).toBeDisabled()
    expect(screen.getByRole('button', { name: 'Cancel' })).toBeDisabled()
  })
})
