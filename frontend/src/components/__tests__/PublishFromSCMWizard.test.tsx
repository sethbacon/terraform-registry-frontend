...
function renderWizard(props: Partial<{ moduleId: string; moduleSystem: string; onComplete: () => void; onCancel: () => void }> = {}) {
  return render(
...
describe('PublishFromSCMWizard', () => {
  ...
  it('Cancel button calls onCancel', async () => {
    ...
    const user = userEvent.setup();
    render(
      <PublishFromSCMWizard moduleId="mod-1" onCancel={onCancel} />,
    );
    await waitFor(() => expect(screen.getByText('Select SCM Provider')).toBeInTheDocument());
  ...
  it('calls linkModuleToSCM with sync_all settings', async () => {
    ...
    const user = userEvent.setup();
    render(
      <PublishFromSCMWizard moduleId="mod-1" onComplete={onComplete} />,
    );
    await waitFor(() => expect(screen.getByText('Select SCM Provider')).toBeInTheDocument());
    ...
    expect(api.linkModuleToSCM).toHaveBeenCalledWith('mod-1', expect.objectContaining({
      provider_id: 'prov-1',
