...
The **Dev Login** button appears because the frontend image is built with
`VITE_MODE=development`, and it works because the backend runs with `DEV_MODE=true`
(which exposes `/api/v1/dev/login`). Both are set in `docker-compose.yml`, so no
identity provider is needed. Setup is not required; the banner does not appear in DEV_MODE.
...
Adds a **Keycloak** identity provider, pre-configured with a test realm and 6 users.
...
First create the external `shared-keycloak` network once (idempotent — this is the
network shared with the state-manager stack so it can reach the same Keycloak):
...
Keycloak serves HTTPS only (HTTP is disabled) on port 8443 using a self-signed dev
cert generated by `keycloak/generate-dev-certs.ps1`. Accept the cert warning once,
and ensure the hosts-file entry below is present so `keycloak` resolves.

| Service             | URL                                                                                |
| ------------------- | ---------------------------------------------------------------------------------- |
| Registry (frontend) | <https://registry.local:3000> (accept self-signed cert warning once)               |
| Keycloak admin      | <https://keycloak:8443/admin> (admin / admin)                                       |
| OIDC discovery      | <https://keycloak:8443/realms/terraform-registry/.well-known/openid-configuration> |
...
| carol.auditor | <carol@example.com> | Carol Auditor | none             |
| dave.manager  | <dave@example.com>  | Dave Manager  | none             |
...
2. Click **Login** — you will be redirected to the Keycloak login page at `keycloak:8443`
...
| `docker-compose.contract-check.yml` | Contract-check stack used by the CI contract-check job            |
| `keycloak/realm-export.json`      | Keycloak realm definition: client, users, mappers                   |
| `keycloak/tsm-realm-export.json`  | State-manager realm definition (mounted by the OIDC stack)          |
| `keycloak/seed-oidc-dev.sql`      | SQL seed: marks setup complete, provisions <admin@example.com>        |
| `keycloak/generate-dev-certs.ps1` | Generates the local dev CA + Keycloak TLS cert                      |
| `certs/`                          | Generated dev-cert artifacts (CA bundle, Keycloak key/cert)         |
